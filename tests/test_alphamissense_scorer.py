"""Tests for the AlphaMissense scorer (offline, --mock mode only)."""
...
SCORER = REPO / "models" / "alphamissense" / "score.py"
...
def test_mock_scorer_contract_and_determinism(tmp_path):
    ...
    assert "alphamissense" in df1.columns
    assert len(df1) == 3
    assert {"variant_id", "chrom", "pos", "ref", "alt"} <= set(df1.columns)
    assert df1["alphamissense"].between(0, 1).all()
